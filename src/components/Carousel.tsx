...
const Carousel = ({ projectData }: { projectData: Project[] }) => {
  ...
  return (
    <div className="carousel-container w-4/5 mx-auto">
            ...
            <div key={`${project.description}-${idx}`} className="flex flex-col w-80 h-auto">
              <div className="relative rounded-lg overflow-hidden shadow-lg border-solid border-slate-300 border-2">
                ...
                <a href={project.link} target="_blank" rel="noopener noreferrer"
                  className="absolute rounded-full bottom-4 left-1/2 transform -translate-x-1/2 px-6 py-2 bg-white poppins-light
                  border-solid border-slate-300 border-2 text-slate-600 hover:bg-slate-400 hover:text-white transition
                  text-sm">
                  Deployment
                </a>
              </div>
      ...
      <button
        className="flex items-center justify-center carousel-control carousel-control-prev rounded-full"
        onClick={prevSlide}
      >
        <img className="h-5" src="right.png" alt="Previous" />
      </button>
      <button
        className="flex items-center justify-center carousel-control carousel-control-next rounded-full"
        onClick={nextSlide}
      >
