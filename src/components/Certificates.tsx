...
const Certificates = ({certificates}: {certificates:Cert[]}) => {
  ...
  return (
    <div className="carousel-container mt-7">
        <span class="text-white text-xl md:text-2xl mb-3 mt-4 md:my-10 md:mt-4 font-normal">
        Certificates
        </span>
      <div className="carousel-slides flex space-x-4 mt-4">
        {certificates.slice(currentIndex, currentIndex + slidesToShow).map((cert, idx) => (
            <div key={`${cert.description}-${idx}`} className="flex flex-col h-auto w-auto">
              <div className="relative rounded-lg overflow-hidden shadow-lg h-full">
                <img src={cert.root} alt={cert.description} className="w-auto h-96 object-cover"/>
                ...
                <a href={cert.link} target="_blank" rel="noopener noreferrer"
                  ...
                  text-slate-200 hover:bg-slate-200 hover:text-slate-600 transition
                  text-sm">
                  Verify</a>
              </div>
